...
use SinticBolivia\SBFramework\Modules\Invoices\Classes\Siat\Invoices\ElectronicaAlcanzadaIce;
use SinticBolivia\SBFramework\Modules\Invoices\Classes\Siat\Invoices\InvoiceDetailIce;
use SinticBolivia\SBFramework\Modules\Invoices\Classes\Siat\Invoices\AlcanzadaIce;
...
use SinticBolivia\SBFramework\Modules\Invoices\Classes\Siat\Invoices\ComercializacionHidrocarburos;
...
class SiatFactory
{
	...
	public static function construirFacturaSector(int $documentoSector, int $modalidad, &$detailClass)
	{
		...
		if( $modalidad == ServicioSiat::MOD_ELECTRONICA_ENLINEA )
		{
			...
			{
				$factura = new ElectronicaAlcanzadaIce();
				$detailClass = InvoiceDetailIce::class;
			}
			elseif( $documentoSector == DocumentTypes::FACTURA_ALQUILER_INMUEBLES )
			{
				$factura = new ElectronicaAlquilerBienInmueble();
				$detailClass = InvoiceDetailAlquiler::class;
			}
			elseif( $documentoSector == DocumentTypes::FACTURA_TELECOMUNICACIONES )
			{
				$factura = new ElectronicaTelecomunicaciones();
			}
			elseif( $documentoSector == DocumentTypes::FACTURA_PREVALORADA )
			{
		...
		else
		{
			...
			else if( $documentoSector == DocumentTypes::FACTURA_COM_HIDROCARBUROS )
			{
				$factura = new ComercializacionHidrocarburos();
			}
			else if( $documentoSector == DocumentTypes::FACTURA_HOTELES )
			{
				$factura = new Hotel();
