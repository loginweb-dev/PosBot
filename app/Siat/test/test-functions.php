...
function construirFactura($codigoPuntoVenta = 0, $codigoSucursal = 0, $modalidad = 0, $documentoSector = 1, $codigoActividad = '620100', $codigoProductoSin = '')
{
	...
	$factura 		= SiatFactory::construirFacturaSector($documentoSector, $modalidad, $detailClass);
	
	for($i = 0; $i < ($documentoSector == DocumentTypes::FACTURA_PREVALORADA ? 1 : 2); $i++)
	{
		$detalle = $factura->instanceDetail(); //new $detailClass();
		$detalle->cantidad				= 1;
		...
		elseif( $documentoSector == DocumentTypes::FACTURA_PRODS_ICE )
		{
			$detalle->marcaIce = 1;
			//$detalle->montoIceEspecifico = 10;
		}
	...
	elseif( $documentoSector == DocumentTypes::FACTURA_COM_HIDROCARBUROS )
	{
		$factura->cabecera->codigoPais 			= 200;
		$factura->cabecera->placaVehiculo 		= 'LUV3421';
		$factura->cabecera->tipoEnvase 			= 'Bidon';
		$factura->cabecera->codigoAutorizacionSC 	= null;
		$factura->cabecera->observacion 			= null;
		$factura->cabecera->montoTotalSujetoIva	= $factura->cabecera->montoTotal * 0.70;
	}
	...
	else if( $documentoSector == DocumentTypes::FACTURA_PRODS_ICE )
	{
		//$factura->cabecera->montoIceEspecifico = 0;
	}
	return $factura;
}
...
function obtenerUltimoCufd($nit, $sucursal, $puntoventa)
{
	
	$filename = sprintf("%s/nit-%d-lcuf-%d-%d.json", __DIR__, $nit, $sucursal, $puntoventa);
	if( !is_file($filename) )
		return null;
	
	$cufd = json_decode(file_get_contents($filename));
	if( !$cufd )
		return null;
	$limit_time = strtotime($cufd->RespuestaCufd->fechaVigencia);
	//die(date('Y-m-d H:i:s', $limit_time));
	if( time() > $limit_time )
		return null;
	return $cufd;
}
function obtenerCufd($codigoPuntoVenta, $codigoSucursal, $cuis, $new = false)
{
	...
	$lastCufd = $new ? null : obtenerUltimoCufd($config->nit, $codigoSucursal, $codigoPuntoVenta);
	if( $lastCufd )
	{
		$resCufd = $lastCufd;
		return $lastCufd;
	...
	$serviceCodigos = new ServicioFacturacionCodigos();
	$serviceCodigos->setConfig((array)$config);
	$serviceCodigos->cuis = $cuis;
	$resCufd = $serviceCodigos->cufd($codigoPuntoVenta, $codigoSucursal);
	
	//##save last CUFD
	$filename = sprintf("%s/nit-%d-lcuf-%d-%d.json", __DIR__, $config->nit, $codigoSucursal, $codigoPuntoVenta);
	file_put_contents($filename, json_encode($resCufd));
...
function obtenerTipoFactura($documentoSector)
{
	$noIva = [DocumentTypes::FACTURA_COM_EXPORT_SERVICIOS, 
	...
	if( in_array($documentoSector, $noIva) )
		return SiatInvoice::FACTURA_SIN_DERECHO_CREDITO_FISCAL;
	
	return SiatInvoice::FACTURA_DERECHO_CREDITO_FISCAL;
}